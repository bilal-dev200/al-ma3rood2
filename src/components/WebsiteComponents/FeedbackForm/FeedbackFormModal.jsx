...
export default function FeedbackFormModal({ isOpen, onClose }) {
      ...
      case "text":
        return (
          <textarea
            ...
            onChange={(e) => handleAnswerChange(question.id, e.target.value)}
            rows={4}
            className="w-full border-2 border-gray-200 rounded-lg px-4 py-3 text-sm focus:ring-2 focus:ring-green-500 focus:border-green-500 text-gray-800 resize-none"
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm p-4">
      <div className={`bg-white rounded-xl w-full max-w-2xl max-h-[90vh] shadow-2xl relative transition-all flex flex-col ${isArabic ? "text-right" : "text-left"}`}>
        {/* Close Button */}
        <button
          onClick={handleClose}
          ...
          {loading ? (
            ...
            <>
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                ...
                {/* Action Buttons */}
                <div className={`flex gap-3 pt-4 ${isArabic ? "justify-start" : "justify-end"}`}>
                  <Button
                    title={t("Cancel")}
                    onClick={handleClose}
